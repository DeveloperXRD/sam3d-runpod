...
print("[boot] Primitive Fitting boot starting...", flush=True)
...
try:
    ...
    subprocess.check_call([sys.executable, "-m", "pip", "install", "-q",
        ...
        "cadquery>=2.4", "scipy>=1.11", "trimesh>=4.0", "Pillow>=10.0"])
    print("[boot] Dependencies installed!", flush=True)

# Run the handler
hp = "/workspace/handler.py"
if os.path.exists(hp):
    print("[boot] Starting Primitive Fitting HTTP server...", flush=True)
    import runpy
    runpy.run_path(hp, run_name="__main__")
else:
    print(f"[boot] ERROR: {hp} not found!", flush=True)
    print("[boot] Upload primitive-fit handler.py to /workspace/handler.py", flush=True)
    sys.exit(1)
